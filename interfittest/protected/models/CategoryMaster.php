...
class CategoryMaster extends CActiveRecord
{
	 ...
	 */
	  public $imagePath = 'category/';
	public $imagePathThumb = 'category/thumb/';
	...
	public function rules()
	{
		...
		return array(
			array('category_name, user_master_user_id', 'required'),
			array('status, user_master_user_id', 'numerical', 'integerOnly'=>true),
			array('category_name, image', 'length', 'max'=>45),
			array('description, date_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, category_name, description, image, date_time, status, user_master_user_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'userMasters' => array(self::MANY_MANY, 'UserMaster', 'category_master_has_user_master(category_master_category_id, user_master_user_id)'),
			'groupMasters' => array(self::HAS_MANY, 'GroupMaster', 'category_master_id'),
		);
	}
